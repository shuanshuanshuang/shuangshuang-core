...
import com.dongnaoedu.mall.manager.mapper.TbOrderShippingMapper;
import com.dongnaoedu.mall.manager.mapper.TbThanksMapper;
import com.dongnaoedu.mall.manager.pojo.TbOrder;
import com.dongnaoedu.mall.manager.pojo.TbOrderExample;
import com.dongnaoedu.mall.manager.pojo.TbOrderItem;
import com.dongnaoedu.mall.manager.pojo.TbOrderItemExample;
...
public class OrderServiceImpl implements OrderService {
	...
	public int passPay(String tokenName, String token, String id) {
		...
		if (StringUtils.isNotBlank(tbThanks.getEmail()) && EmailUtil.checkEmail(tbThanks.getEmail())) {
			String content = "您的订单已支付成功，十分感谢您的捐赠！<br>您可以在捐赠名单中查看到您的数据："
					+ "<a href='http://localhost:5555/#/thanks'>http://localhost:5555/#/thanks</a><br>";
	...
	public int backPay(String tokenName, String token, String id) {
		...
		// 发送通知邮箱
		if (StringUtils.isNotBlank(tbThanks.getEmail()) && EmailUtil.checkEmail(tbThanks.getEmail())) {
			String content = "抱歉，由于您支付不起或其他原因，您的订单支付失败，请尝试重新支付！<br>";
	...
	public int notShowPay(String tokenName, String token, String id) {
		...
		// 发送通知邮箱
		if (StringUtils.isNotBlank(tbThanks.getEmail()) && EmailUtil.checkEmail(tbThanks.getEmail())) {
			String content = "您的订单已支付成功，十分感谢您的捐赠！<br>但由于您的支付金额过低或其他原因，将不会在捐赠名单中显示，敬请谅解！<br>";
			emailUtil.sendEmailPayResult(tbThanks.getEmail(), "【电商商城】支付捐赠成功通知", content);
		}
		return 1;
	}
	...
	public int editPay(String tokenName, String token, TbThanks tbThanks) {
		...
		// 验证token
		if (StringUtils.isBlank(tokenName) || StringUtils.isBlank(tokenName)
				|| StringUtils.isBlank(tbThanks.getId().toString())) {
			return -1;
		}
		String value = jedisClient.get(tokenName);
		if (!value.equals(token)) {
