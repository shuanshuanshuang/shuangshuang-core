...
import com.dongnaoedu.mall.common.utils.TimeUtil;
import com.dongnaoedu.mall.manager.dto.OrderChartData;
import com.dongnaoedu.mall.manager.mapper.TbOrderMapper;
import com.dongnaoedu.mall.manager.service.CountService;
...
public class CountServiceImpl implements CountService {
	...
	public List<OrderChartData> getOrderCountData(int type, Date startTime, Date endTime, int year) {
		...
		if (type == CountConstant.THIS_WEEK) {
			// 本周
			List<OrderChartData> data = tbOrderMapper.selectOrderChart(TimeUtil.getBeginDayOfWeek(),
					TimeUtil.getEndDayOfWeek());
			fullData = getFullData(data, TimeUtil.getBeginDayOfWeek(), TimeUtil.getEndDayOfWeek());
		} else if (type == CountConstant.THIS_MONTH) {
			// 本月
			List<OrderChartData> data = tbOrderMapper.selectOrderChart(TimeUtil.getBeginDayOfMonth(),
					TimeUtil.getEndDayOfMonth());
			fullData = getFullData(data, TimeUtil.getBeginDayOfMonth(), TimeUtil.getEndDayOfMonth());
		} else if (type == CountConstant.LAST_MONTH) {
			// 上个月
			List<OrderChartData> data = tbOrderMapper.selectOrderChart(TimeUtil.getBeginDayOfLastMonth(),
					TimeUtil.getEndDayOfLastMonth());
